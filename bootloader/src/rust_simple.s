...
long_mode_64:
    ...
    ; 显示Rust内核测试消息
    mov rsi, rust_test_msg
    call print_string_64

    ; 创建一个简单的64位内核代码在内存中
    call create_simple_kernel

    ; 显示跳转消息
    mov rsi, jumping_simple_msg
    call print_string_64

    ; 跳转到简单内核
    mov rax, 0x20000    ; 简单内核地址
    jmp rax
...
create_simple_kernel:
    ...
    ; 创建简单的64位代码：显示"KERN"然后无限循环
    ; mov rdi, 0xb8000 + 3200 (第20行) (48 bf 80 8c 0b 00 00 00 00 00)
    mov al, 0x48
    stosb
    mov al, 0xbf
    stosb
    ...
    ; mov word [rdi], 0x0e4b (66 c7 07 4b 0e) - 'K' 黄色
    mov al, 0x66
    stosb
    mov al, 0xc7
    stosb
    ...
    ; mov word [rdi+2], 0x0e45 (66 c7 47 02 45 0e) - 'E' 黄色
    mov al, 0x66
    stosb
    mov al, 0xc7
    stosb
    ...
    ; mov word [rdi+4], 0x0e52 (66 c7 47 04 52 0e) - 'R' 黄色
    mov al, 0x66
    stosb
    mov al, 0xc7
    stosb
    ...
    ; mov word [rdi+6], 0x0e4e (66 c7 47 06 4e 0e) - 'N' 黄色
    mov al, 0x66
    stosb
    mov al, 0xc7
    stosb
...
rust_test_msg: db "Rust Kernel Test: READY!", 0
jumping_simple_msg: db "Jumping to Simple Kernel...", 0
jumping_rust_kernel_msg: db "Jumping to Rust Kernel...", 0
loading_rust_32_msg: db "Loading Rust Kernel (32-bit)...", 0
rust_loaded_32_msg: db "Rust Kernel Loaded (32-bit): OK", 0
